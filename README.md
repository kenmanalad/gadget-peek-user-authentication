...
It offers **secure registration, login, token management, email verification, Google OAuth login, password reset, account deactivation, admin role management, and account updates** with robust validation, error handling, and rate limiting.
...
#### **8.1 Email Address Update**
**Authentication**: Requires `JwtAuthGuard` with valid `access_token`.

**Two-step process**:

**Send Code**:
- Sends verification code to the new email address.
- Saves the code in the database linked to the authenticated user.

**Verify & Update**:
- Validates the submitted code and verifies the `access_token`.
- Updates the email address if validation succeeds.

**Security**:
- Rate limiting via Redis.
- `JwtAuthGuard` authentication.
- Prisma & HTTP exception filters.
- DTO validation & sanitization.
...
**Process**:
- Accepts old password and new password.
- Verifies that the old password matches the registered hashed password.
